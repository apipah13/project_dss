...
cv_scores = cross_val_score(
    model, X_train, y_train, cv=tscv, scoring="accuracy", n_jobs=-1
)
...
if gap > 0.10:
    print(f"Overfitting terdeteksi! Gap: {gap:.4f}")
else:
...
fig.suptitle(
    "Random Forest - Evaluasi Model Cuaca",
...
lag_features = [
    "hujan_kemarin", "suhu_kemarin", "kelembapan_kemarin",
    "suhu_2halu", "kelembapan_2halu"
]
colors = ["#FF9800" if f in lag_features else "#2196F3" for f in feat_imp.index]
...
ax1.set_title(
    "Feature Importance (Biru = Fitur Utama, Oranye = Fitur Lag)",
    fontweight="bold"
)
ax1.axvline(
    feat_imp.mean(), color="red", linestyle="--", alpha=0.7,
    label=f"Rata-rata: {feat_imp.mean():.3f}"
)
ax1.legend()
...
ax3.axhline(
    cv_scores.mean(), color="navy", linestyle="--",
